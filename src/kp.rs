...
// TODO: not clear how to marry the concept of individual line widths with this approach b/c of flushes

define_language! {
  enum Doc {
    "glue" = Glue([Id; 3]), // glue takes an ideal width, stretchability, shrinkability
    "penalty" = Penalty([Id; 3]), // width and penalty and flagged
    "flush" = Flush(Id),
    "<|>" = Choice([Id; 2]),
    // TODO: these operators (except text) are derived, and don't really need to be in the e-graph!
    "$$" = VCat([Id; 2]),
    "inf" = Infinity,
    // problem: negative infinities shouldn't absorb each other, but should always be taken I think.
    // pointwise comparison not reducing global cost. maybe count up infinities as part of the cost?
    "neginf" = NegativeInfinity,
    Number(u32),
    Flagged(bool),
    Text(Symbol), // for now, box is text
  }
}
